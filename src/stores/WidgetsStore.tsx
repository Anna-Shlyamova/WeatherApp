        ...
        layout :
          <>
              {WeatherStore.currentWeather?.temp_c
                  ? (<Typography sx={theme => ({color: theme.palette.primary.contrastText, fontSize: '48px'})}>{Math.round(WeatherStore.currentWeather?.temp_c)}&deg;C</Typography>)
                  : <Loader/>
              }
          </>,
        previewLayout:
        ...
        fullLayout:
          <>
            {WeatherStore.forecastCurrentHoursWeather
              ? (
                <Line data={this.getChartData('temp_c', 'Температура')} options={this.getChartOptions('Температура', '°C')} width={'700px'} height={'400'}/>
              )
              : <Loader/>
            }
          </>,
      },
      {
        ...
        layout :
          <>
            {WeatherStore.currentWeather?.wind_kph
              ? (<Typography sx={theme => ({color: theme.palette.primary.contrastText, fontSize: '48px'})}>{WeatherStore.currentWeather?.wind_kph} км/ч</Typography>)
              : <Loader/>
            }
          </>,
        previewLayout:
        ...
        fullLayout:
          <>
            {WeatherStore.forecastCurrentHoursWeather
              ? (
                <Line data={this.getChartData('wind_kph', 'Ветер')} options={this.getChartOptions('Ветер', 'км/ч')} width={'700px'} height={'400'}/>
              )
              : <Loader/>
            }
          </>,
      },
      {
        ...
        fullLayout:
          <>
            {WeatherStore.forecastCurrentHoursWeather
              ? (
                <Line data={this.getChartData('humidity', 'Влажность')} options={this.getChartOptions('Влажность', '%')} width={'700px'} height={'400'}/>
              )
              : <Loader/>
            }
          </>,
      },
      {
        ...
        fullLayout:
          <>
            {WeatherStore.forecastCurrentHoursWeather
              ? (
                <Line data={this.getChartData('uv', 'УФ-Индекс')} options={this.getChartOptions('УФ-Индекс')} width={'700px'} height={'400'}/>
              )
              : <Loader/>
            }
          </>,
